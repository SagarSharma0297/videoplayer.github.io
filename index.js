var statusCheck = window.localStorage.getItem('login-status');
if(statusCheck=== "true"){
...
var loginButton = window.document.getElementById('login-button');

  function makeCard(data) {
    var anchorTag = window.document.createElement('a');
    if(statusCheck === "true"){
        anchorTag.href = "./player.html?id="+data.id;
    }else{
        anchorTag.href = "./loginpage.html"
    }
   
    anchorTag.className = "acard";
    var card=document.createElement('div');
    var imageTag=document.createElement('img')
    card.appendChild(imageTag);
    imageTag.src=data.thumbnail;
    imageTag.className="img-grid";
    card.className="card";
    var title = document.createElement('div');
    title.className="text";
    title.innerHTML=data.title;
    card.appendChild(title);
    anchorTag.appendChild(card);
...
xhttp.onreadystatechange = () => {
    if(xhttp.readyState === 4){
        newVideoArr = JSON.parse(xhttp.responseText);
        for(var i=0;i<newVideoArr.length;i++) {
            makeCard(newVideoArr[i])
        }
    }
};

  /* loginButton.onchange = function () {
  console.log('loginbutton chNGE')
